...
class Human {
    ...
    func simulate(_ cycle:Int){
        ...
        tissues.amountOfCO2 = tissues.amountOfCO2 + (0.98 * heart.decilitersPerIteration * (arteries.effluentCO2Content-tissues.carbonDioxideContent) + tissues.respiratoryQuotient * u) * 0.001
        ...
        tissues.TC3AJ = dampChange(0.2*tissues.referenceCO2, oldValue:tissues.TC3AJ, dampConstant:c10)
        ...
        tissues.bicarbonateContent = tissues.bicarbonateAmount*c13 + (tissues.pCO2-40.0)*c3 - tissues.TC3AJ
        if ((tissues.bicarbonateContent * tissues.pCO2) <= 0) {print("We have a bad fucking arithmetic problem in tissue bicarb!")}
        tissues.updatePh()
        tissues.calculateContents()
        ...
        if (venousPool.addBicarb > 0){
            venousPool.amountOfCO2 = venousPool.amountOfCO2 + 22.4 * venousPool.addBicarb}
        ...
        venousPool.amountOfCO2 = venousPool.amountOfCO2 + 0.98*heart.decilitersPerIteration*tissues.carbonDioxideContent-heart.decilitersPerIteration*(venousPool.carbonDioxideContent*c14 - arteries.carbonDioxideContent*heart.leftToRightShunt)+x*arteries.effluentCO2Content
        ...
        venousPool.amountOfOxygen = venousPool.amountOfOxygen + 0.98*heart.decilitersPerIteration*tissues.oxygenContent-heart.decilitersPerIteration*(venousPool.oxygenContent*c14 - arteries.oxygenContent*heart.leftToRightShunt)+x*arteries.effluentOxygenContent
        
        // Adjust tissue and venous pool bicarb amounts - bicarb moves after delay line into arterial side
        xnew = heart.fitnessAdjustedOutputPerIteration * 0.1 * (tissues.bicarbonateAmount*c13 - (venousPool.bicarbonateContent - (XC2PR-40)*c3))
        //   xnew = 0.98*[heart.decilitersPerIteration]*0.1*([tissues.bicarbonateAmount]*c13 -([venousPool. bicarbonateContent]-
        //                                                                                               (XC2PR-40)*c3));
        
        venousPool.bicarbonateAmount =  venousPool.bicarbonateAmount + xnew + venousPool.addBicarb
        tissues.bicarbonateAmount = tissues.bicarbonateAmount - xnew
        
        // Do the venous oxygen and CO2 content changes
        venousPool.oxygenContent = venousPool.amountOfOxygen * c2
        venousPool.carbonDioxideContent = venousPool.amountOfCO2 * c2
        
        // Calculate bicarb content after infusion but before delay line
        venousPool.bicarbonateContent = venousPool.bicarbonateAmount * c1 + (tissues.pCO2 - 40) * c3 - tissues.TC3AJ
        XC2PR = tissues.pCO2
        if (venousPool.bicarbonateAmount<=0) {print("We have a big fucking arithmetic problem in venous bicarb!")}
        ...
        n = pntdly + nft
        if (n > 10) {n = n - 10}
        venousPool.oxygenContent = veinDelay[n - 1]
        venousPool.carbonDioxideContent = veinDelay[n+9]
        venousPool.bicarbonateContent = veinDelay[n+19]
        ...
        venousPool.updatePh(venousPool.bicarbonateContent,CO2:XC2PR)
        ...
        lungs.amountOfOxygen = lungs.amountOfOxygen + u
        lungs.amountOfCO2 = lungs.amountOfCO2 + v
        lungs.amountOfNitrogen = lungs.amountOfNitrogen + x * (100 - lungs.FiO2 - lungs.FiCO2)
        ...
        lungs.amountOfOxygen = lungs.amountOfOxygen + pc * (venousPool.oxygenContent-lungs.oxygenContent)
        lungs.amountOfCO2 = lungs.amountOfCO2 + pc * (venousPool.carbonDioxideContent-lungs.carbonDioxideContent)
        lungs.amountOfNitrogen = lungs.amountOfNitrogen + pc * (tissues.pN2 * 0.00127 - arteries.effluentNitrogenContent)
        ...
        if (pl < 0) {
            ...
            print("Call Bager from line 790")
        }
        ...
        lungs.exhaledVentilation = pc*c35 - lungs.totalLungVolume
        
        if (lungs.exhaledVentilation < 0) {
            fd = lungs.exhaledVentilation * c12
            lungs.amountOfOxygen = lungs.amountOfOxygen - fd * lungs.FiO2
            lungs.amountOfCO2 = lungs.amountOfCO2 - fd * lungs.FiCO2
            lungs.amountOfNitrogen = lungs.amountOfNitrogen - fd * z
        }
        else {
            x = lungs.exhaledVentilation*c25
            lungs.totalVentilation = x/pc
            x = x/c11
            y = x * lungs.pO2
            z = x * lungs.pCO2
            ...
            if (pl > 0.5) {
                ...
                print("Call BAGER from line 820")
                if (x < -9000) {print("We have a bad fucking arithmetic problem in Fortran line 820!")}
            }
            
            
            if lungs.alveolarVentilationPerIteration < e {qa = 0.001; qb = e}
            // Set the new amounts of gas
            lungs.amountOfOxygen = ((lungs.amountOfOxygen-y)>0.01) ? lungs.amountOfOxygen - y : 0.01
            lungs.amountOfCO2 = ((lungs.amountOfCO2-z) > 0.01) ? lungs.amountOfCO2 - z : 0.01
            lungs.amountOfNitrogen = ((lungs.amountOfNitrogen-pc)>0.01) ? lungs.amountOfNitrogen - pc : 0.01
        }
        ...
        lungs.updatePh(x, CO2: lungs.pCO2)
        ...
        brain.bloodFlow = dampChange((y-0.12)*42.8*z + Double(brain.C2CHN*7), oldValue: brain.bloodFlow*z, dampConstant: c76)
        
        if (heart.effectiveCardiacOutput*2 > brain.bloodFlow) {brain.bloodFlow = heart.effectiveCardiacOutput*2}
        ...
        brain.amountOfOxygen = brain.amountOfOxygen + y * (arteries.oxygenContent-brain.oxygenContent) - 2 * z * (brain.oxygenationIndex+0.1)
        brain.amountOfOxygen = max(brain.amountOfOxygen, 0.1)
        
        brain.amountOfCO2 = brain.amountOfCO2 + y * (arteries.carbonDioxideContent-brain.carbonDioxideContent) + 2.15 * x
        brain.pO2 = brain.amountOfOxygen*1.6
        brain.pCO2 = brain.amountOfCO2*0.078
        w = brain.pCO2 - 40
        ...
        brain.bicarbDeviation = brain.bicarbDeviation + ((arteries.bicarbonateContent-24) * 0.3 - brain.bicarbDeviation) * c42
        ...
        y = ((118 - pj)*0.05) + brain.C2CHN*0.05 + venousPool.addBicarb*0.2
        z = y * 0.002
        ...
        u = (u < 0) ? 0 : 1
        brain.oxygenationIndex = dampChange(u, oldValue: brain.oxygenationIndex, dampConstant: c63)
        ...
        u = min(u, c6) // restrict to a maximum value stored in c6
        ...
        lungs.effectiveVentResponse = brain.oxygenationIndex * dampChange(u, oldValue: lungs.effectiveVentResponse, dampConstant: x)
        
        
        // NOW A SERIOUS RAT NEST OF FORTRAN IF STATEMENTS:
        
        if pl < 0 {
            lungs.totalVentilation = e
            lungs.respiratoryRate = e
        }
            
        else { // pl > = 0
            if (lungs.effectiveVentResponse < c48) && ventilator.off() {
                lungs.totalVentilation = e
                lungs.respiratoryRate = e
            }
            else { // effectiveVentResponse must be > c48 but the ventilator could be on or off so we test
                if ventilator.off(){
                    lungs.totalVentilation = lungs.effectiveVentResponse
                    lungs.respiratoryRate = (c49 + pow(lungs.totalVentilation, 0.7)*0.37)*c50/(pj+40)
                }
            }
            if (lungs.respiratoryRate > 1) && (heart.effectiveCardiacOutput > 0.5) {
                u = lungs.pO2 * 0.15
                u = (u > 70) ? 70 : u
                lungs.deadSpace = dampChange((c52+lungs.totalVentilation*100 / pow(lungs.respiratoryRate,1.12) + 20 * lungs.totalVentilation/(heart.effectiveCardiacOutput+5)+u+fy+c54*(lungs.tidalVolume+500)), oldValue: lungs.deadSpace, dampConstant: c55)
            }
        }
        
        
        lungs.tidalVolume = lungs.totalVentilation*1000/lungs.respiratoryRate
        if ventilator.off() {
            lungs.tidalVolume = lungs.tidalVolume>c20 ? c20 : lungs.tidalVolume
            lungs.respiratoryRate = lungs.totalVentilation*1000/lungs.tidalVolume
        }
        
        lungs.alveolarVentilationPerIteration = lungs.respiratoryRate*(lungs.tidalVolume - lungs.deadSpace) * ft
        ...
        if cycle == iterations {self.venousPool.addBicarb = 0}
